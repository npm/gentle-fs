...
function testLink (opts, cb) {
  ...
  var link = requireInject('../../lib/link.js', {
    '../../lib/rm.js': dezalgo(function (toRemove, opts) {
      if (opts.rm[toRemove] && opts.rm[toRemove].gently === opts.gently) {
        cb()
      } else {
        cb(new Error('Removing toRemove: ' + util.inspect(toRemove) +
          ' gently: ' + util.inspect(opts.gently) +
  ...
  link.link(opts.from, opts.to, opts, cb)
}
